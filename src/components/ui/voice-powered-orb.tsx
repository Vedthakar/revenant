...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const simulatedLevelRef = useRef(simulatedLevel ?? 0);

  useEffect(() => {
    simulatedLevelRef.current = simulatedLevel ?? 0;
  }, [simulatedLevel]);
  ...
  const frag = /* glsl */ `
    ...
    const vec3 baseColor1 = vec3(1.0, 0.698, 0.365);
    const vec3 baseColor2 = vec3(0.85, 0.45, 0.12);
    const vec3 baseColor3 = vec3(0.18, 0.08, 0.02);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        // Use simulated level if provided, otherwise use mic
        const simLevel = simulatedLevelRef.current;
        let voiceLevel = 0;
      ...
      return () => {
        ...
        if (container && gl && gl.canvas) {
          try { if (container.contains(gl.canvas as Node)) container.removeChild(gl.canvas as Node); } catch {}
        }
        stopMicrophone();
